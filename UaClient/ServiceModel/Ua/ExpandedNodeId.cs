...
{
    public static readonly ExpandedNodeId Null = new(0);
    ...
    public static NodeId ToNodeId(ExpandedNodeId value, IReadOnlyList<string> namespaceUris)
    {
        ...
        if (namespaceUris is null) throw new ArgumentNullException(nameof(namespaceUris));
        if (string.IsNullOrEmpty(value.NamespaceUri)) return value.NodeId;
        var ns = -1;
        for (var i = 0; i < namespaceUris.Count; i++)
        ...
        if (ns < 0) throw new IndexOutOfRangeException();
    ...
    public static bool TryParse(string s, [NotNullWhen(true)] out ExpandedNodeId? value)
    {
        try
        {
            uint svr = 0;
            ...
            {
                var pos = s.IndexOf(';');
                if (pos == -1) throw new ServiceResultException(StatusCodes.BadNodeIdInvalid);
    ...
    {
        if (!TryParse(s, out var value)) throw new ServiceResultException(StatusCodes.BadNodeIdInvalid);
    ...
    public override string ToString()
    {
        ...
        if (ServerIndex > 0) sb.AppendFormat("svr={0};", ServerIndex);

        if (!string.IsNullOrEmpty(NamespaceUri)) sb.AppendFormat("nsu={0};", NamespaceUri);
