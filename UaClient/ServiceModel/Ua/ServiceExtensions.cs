...
public static class ServiceExtensions
{
    ...
    /// <returns>A <see cref="Task" /> representing the asynchronous operation.</returns>
    public static async Task<StatusCode> AcknowledgeAsync(this IRequestChannel channel,
        AcknowledgeableCondition condition, LocalizedText? comment = null)
    {
            ...
            MethodsToCall = new[]
            {
                new CallMethodRequest
                {
                    ...
                    InputArguments = new Variant[]
                        { condition.EventId, comment } // ?? new LocalizedText(string.Empty) }
                }
            }
        });
    ...
    /// <param name="comment">a comment.</param>
    /// <returns>A <see cref="Task" /> representing the asynchronous operation.</returns>
    public static async Task<StatusCode> ConfirmAsync(this IRequestChannel channel, AcknowledgeableCondition condition,
        LocalizedText? comment = null)
    {
        if (condition == null) throw new ArgumentNullException(nameof(condition));
    ...
    {
        return task.TimeoutAfter(-1, cancellationToken);
    }


    public static Task TimeoutAfter(this Task task, int millisecondsTimeout, CancellationToken token = default)
    {
        return task.TimeoutAfter(TimeSpan.FromMilliseconds(millisecondsTimeout), token);
    }
    ...
    public static async Task TimeoutAfter(this Task task, TimeSpan timeout, CancellationToken token = default)
    {
        ...
        else if (!t.IsCanceled) throw new TimeoutException($"Task timed out after {timeout}");
        ...
        CancellationToken token)
    {
        ...
        if (task != t)
        {
            if (!t.IsCanceled) throw new TimeoutException($"Task timed out after {timeout}");
