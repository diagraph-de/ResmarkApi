...
{
    public static readonly ServiceResult Good = new(StatusCodes.Good);
    ...
    public ServiceResult(StatusCode code, string? symbolicId = null, string? namespaceUri = null,
        LocalizedText? localizedText = null, string? additionalInfo = null, ServiceResult? innerResult = null)
    {
        StatusCode = code;
    ...
    public ServiceResult(StatusCode code, DiagnosticInfo? diagnosticInfo, IList<string?>? stringTable)
    {
        ...
        if (diagnosticInfo != null)
        {
            ...
            var locale = LookupString(stringTable, diagnosticInfo.Locale);
            var localizedText = LookupString(stringTable, diagnosticInfo.LocalizedText);
            LocalizedText = new LocalizedText(localizedText, locale);

            AdditionalInfo = diagnosticInfo.AdditionalInfo;

            if (!StatusCode.IsGood(diagnosticInfo.InnerStatusCode))
                InnerResult = new ServiceResult(diagnosticInfo.InnerStatusCode, diagnosticInfo.InnerDiagnosticInfo,
                    stringTable);
        }
    }
    ...
    public ServiceResult(StatusCode code, int index, DiagnosticInfo[] diagnosticInfos, IList<string?>? stringTable)
    {
        ...
        {
            var diagnosticInfo = diagnosticInfos[index];
    ...
    {
        if (status != null) return StatusCode.IsGood(status.StatusCode);
    ...
    {
        if (status != null) return StatusCode.IsUncertain(status.StatusCode);
    ...
    {
        if (status != null) return StatusCode.IsBad(status.StatusCode);
    ...
    public override string ToString()
    {
        ...
        if (!string.IsNullOrEmpty(SymbolicId))
        {
            ...
            else if (SymbolicId != buffer.ToString()) buffer.AppendFormat(" ({0})", SymbolicId);
        }

        if (!string.IsNullOrEmpty(LocalizedText)) buffer.AppendFormat(" '{0}'", LocalizedText);

        if ((0x0000FFFF & StatusCode) != 0) buffer.AppendFormat(" [{0:X4}]", 0x0000FFFF & StatusCode);
    ...
    {
        if (stringTable == null || index < 0 || index >= stringTable.Count) return null;
