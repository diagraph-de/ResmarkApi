...
///     <para>
///         Assemblies marked with the <see cref="TypeLibraryAttribute" /> are searched for types with
///         <see cref="DataTypeIdAttribute" />.
///     </para>
/// </summary>
public class TypeLibrary
{
    private static readonly Lazy<TypeLibrary> _instance = new(() => new TypeLibrary());
    private readonly Dictionary<Type, ExpandedNodeId> _binaryEncodingIdByType;
    private readonly Dictionary<Type, ExpandedNodeId> _dataTypeIdByType;
    private readonly Dictionary<ExpandedNodeId, Type> _typeByBinaryEncodingId;
    private readonly Dictionary<ExpandedNodeId, Type> _typeByDataTypeId;
    ...
    public static bool TryGetTypeFromDataTypeId(ExpandedNodeId id, [NotNullWhen(true)] out Type? type)
    {
        return _instance.Value._typeByDataTypeId.TryGetValue(id, out type);
    }

    public static bool TryGetDataTypeIdFromType(Type type, [NotNullWhen(true)] out ExpandedNodeId? id)
    {
        return _instance.Value._dataTypeIdByType.TryGetValue(type, out id);
    }

    public static bool TryGetTypeFromBinaryEncodingId(ExpandedNodeId id, [NotNullWhen(true)] out Type? type)
    {
        return _instance.Value._typeByBinaryEncodingId.TryGetValue(id, out type);
    }

    public static bool TryGetBinaryEncodingIdFromType(Type type, [NotNullWhen(true)] out ExpandedNodeId? id)
    {
        return _instance.Value._binaryEncodingIdByType.TryGetValue(type, out id);
    }
}