...
{
    private readonly List<Action<ILoggerFactory>> configureLoggerFactoryActions = new();
    private readonly List<Action<UaApplicationOptions>> configureOptionsActions = new();
    private readonly List<MappedEndpoint> mappedEndpoints = new();
    ...
    /// </param>
    /// <returns>The <see cref="UaApplicationBuilder" />.</returns>
    public UaApplicationBuilder SetDirectoryStore(string path, bool acceptAllRemoteCertificates = true,
        bool createLocalCertificateIfNotExist = true)
    {
        if (string.IsNullOrEmpty(path)) throw new ArgumentNullException(nameof(path));

        certificateStore = new DirectoryStore(path, acceptAllRemoteCertificates, createLocalCertificateIfNotExist);
        return this;
    }
    ...
    /// </summary>
    /// <param name="identity">
    ///     The user identity. Provide an <see cref="AnonymousIdentity" />, <see cref="UserNameIdentity" />,
    ///     <see cref="IssuedIdentity" /> or <see cref="X509Identity" />.
    /// </param>
    /// <returns>The <see cref="UaApplicationBuilder" />.</returns>
    ...
    {
        configureLoggerFactoryActions.Add(configureDelegate ??
    ...
    {
        configureOptionsActions.Add(configureDelegate ?? throw new ArgumentNullException(nameof(configureDelegate)));
        return this;
    }
    ...
    {
        if (string.IsNullOrEmpty(requestedUrl)) throw new ArgumentNullException(nameof(requestedUrl));

        if (endpoint == null) throw new ArgumentNullException(nameof(endpoint));

        mappedEndpoints.Add(new MappedEndpoint { RequestedUrl = requestedUrl, Endpoint = endpoint });
        return this;
    }
    ...
    /// <returns>The <see cref="UaApplicationBuilder" />.</returns>
    public UaApplicationBuilder AddMappedEndpoint(string requestedUrl, string endpointUrl,
        string? securityPolicyUri = null)
    {
        ...
        mappedEndpoints.Add(new MappedEndpoint
        {
            RequestedUrl = requestedUrl,
            Endpoint = new EndpointDescription { EndpointUrl = endpointUrl, SecurityPolicyUri = securityPolicyUri }
        });
        return this;
    }
    ...
    public UaApplication Build()
    {
        ...
        foreach (var action in configureLoggerFactoryActions) action(loggerFactory);

        var options = this.options ?? new UaApplicationOptions();
        foreach (var action in configureOptionsActions) action(options);
