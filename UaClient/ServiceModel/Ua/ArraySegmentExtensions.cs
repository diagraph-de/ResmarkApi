    ...
    public static void CopyTo<T>(this ArraySegment<T> segment, ArraySegment<T> destination)
    {
        ...
        if (destination.Array is null)
            throw new InvalidOperationException(
                "The Array property of the destination array segment may not be null here");

        Array.Copy(segment.Array, segment.Offset, destination.Array, destination.Offset,
            Math.Min(segment.Count, destination.Count));
    }
