...
public class UaApplication : IDisposable
{
    ...
    private readonly TaskCompletionSource<bool> completionTask = new();

    private readonly ILogger? logger;
    private bool disposed;
    private volatile TaskCompletionSource<bool> suspensionTask = new();
    ...
    /// <summary>
    ///     Gets an asynchronous function that provides the identity of the user. Supports <see cref="AnonymousIdentity" />,
    ///     <see cref="UserNameIdentity" />, <see cref="IssuedIdentity" /> and <see cref="X509Identity" />.
    /// </summary>
    ...
    public async Task SuspendAsync()
    {
        ...
        if (suspensionTask.Task.IsCompleted) suspensionTask = new TaskCompletionSource<bool>();
    ...
    public async Task<ClientSessionChannel> GetChannelAsync(string endpointUrl, CancellationToken token = default)
    {
        ...
        var ch = await channelMap
            .GetOrAdd(endpointUrl,
                k => new Lazy<Task<ClientSessionChannel>>(() => Task.Run(() => CreateChannelAsync(k, token))))
            .Value
