...
public class ClientSecureChannel : ClientTransportChannel, IRequestChannel, IEncodingContext
{
    ...
    private readonly SemaphoreSlim _receivingSemaphore = new(1, 1);
        ...
        : base(remoteEndpoint, loggerFactory, options, stackProfile)
    {
        ...
        _pendingRequests = new ActionBlock<ServiceOperation>(t => SendRequestActionAsync(t),
            new ExecutionDataflowBlockOptions { CancellationToken = _channelCts.Token });
    ...
    public virtual async Task<IServiceResponse> RequestAsync(IServiceRequest request, CancellationToken token = default)
    {
        ...
        using (var linkedCts =
               CancellationTokenSource.CreateLinkedTokenSource(timeoutCts.Token, _channelCts.Token, token))
        using (var registration = linkedCts.Token.Register(
                   o => ((ServiceOperation)o!).TrySetException(GetPendingException() ??
                                                               new ServiceResultException(StatusCodes
                                                                   .BadRequestTimeout)), operation, false))
        {
            if (_pendingRequests.Post(operation)) return await operation.Task.ConfigureAwait(false);
    ...
    /// <inheritdoc />
    /// <seealso href="https://reference.opcfoundation.org/v104/Core/docs/Part4/5.5.2/">
    ...
    protected override async Task OnOpenAsync(CancellationToken token = default)
    {
        ...
        _conversation = await StackProfile.ConversationProvider
            .CreateAsync(RemoteEndpoint, LocalDescription, options, CertificateStore, _logger, token)
        ...
        var openSecureChannelResponse =
            (OpenSecureChannelResponse)await RequestAsync(openSecureChannelRequest).ConfigureAwait(false);
    ...
    /// <inheritdoc />
    /// <seealso href="https://reference.opcfoundation.org/v104/Core/docs/Part4/5.5.3/">
    ///     OPC UA specification Part 4: Services,
    ///     5.5.3
    /// </seealso>
    protected override async Task OnCloseAsync(CancellationToken token = default)
    {
        try
        {
            ...
            {
                RequestHeader = new RequestHeader { TimeoutHint = TimeoutHint, ReturnDiagnostics = DiagnosticsHint }
            };
    ...
    private async Task SendRequestAsync(ServiceOperation operation, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            if (DateTime.UtcNow > _tokenRenewalTime)
            {
                ...
                _logger?.LogTrace(
                    $"Sending {openSecureChannelRequest.GetType().Name}, Handle: {openSecureChannelRequest.RequestHeader.RequestHandle}");
                _pendingCompletions.TryAdd(openSecureChannelRequest.RequestHeader.RequestHandle,
                    new ServiceOperation(openSecureChannelRequest));
            ...
            else if (request is CloseSecureChannelRequest closeRequest)
            {
                ...
                operation.TrySetResult(new CloseSecureChannelResponse
                {
                    ResponseHeader = new ResponseHeader
                        { RequestHandle = header.RequestHandle, Timestamp = DateTime.UtcNow }
                });
            }
            else
            {
        ...
        {
            _logger?.LogError(
                $"Error sending {request.GetType().Name}, Handle: {request.RequestHeader!.RequestHandle}. {ex.Message}");
            throw;
        }
        finally
        {
    ...
    private async Task SendOpenSecureChannelRequestAsync(OpenSecureChannelRequest request, CancellationToken token)
    {
        ...
        using (var bodyEncoder = StackProfile.EncodingProvider.CreateEncoder(bodyStream, this, false))
        {
        ...
        try
        {
            while (!token.IsCancellationRequested)
            {
                ...
                if (response == null)
                {
                    ...
                    _channelCts.Cancel();
                    if (State == CommunicationState.Closed || State == CommunicationState.Closing) return;
                ...
                if (_pendingCompletions.TryRemove(header.RequestHandle, out var tcs))
                {
                    ...
                    {
                        var ex = new ServiceResultException(new ServiceResult(header.ServiceResult,
                            header.ServiceDiagnostics, header.StringTable));
                ...
                if (header.RequestHandle == 0)
                {
                    ...
                    if (response is OpenSecureChannelResponse)
                        tcs2 = _pendingCompletions.OrderBy(k => k.Key).Select(k => k.Value)
                            .FirstOrDefault(o => o.Request is OpenSecureChannelRequest);
                    else if (response is CloseSecureChannelResponse)
                        tcs2 = _pendingCompletions.OrderBy(k => k.Key).Select(k => k.Value)
                            .FirstOrDefault(o => o.Request is CloseSecureChannelRequest);
    ...
    private async Task<IServiceResponse?> ReceiveResponseAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            using (var bodyDecoder = StackProfile.EncodingProvider.CreateDecoder(bodyStream, this, false))
            {
                var ret = await _conversation!.DecryptMessageAsync(bodyStream, ReceiveAsync, token)
                    .ConfigureAwait(false);
                ...
                _logger?.LogTrace(
                    $"Received {response.GetType().Name}, Handle: {response.ResponseHeader!.RequestHandle} Result: {response.ResponseHeader.ServiceResult}");
                ...
                if (response is OpenSecureChannelResponse openSecureChannelResponse &&
                    StatusCode.IsGood(openSecureChannelResponse.ResponseHeader!.ServiceResult))
                {
                    _tokenRenewalTime =
                        DateTime.UtcNow.AddMilliseconds(0.8 * openSecureChannelResponse.SecurityToken!.RevisedLifetime);
    ...
    private void TimestampHeader(IServiceRequest request)
    {
        if (request.RequestHeader == null)
        {
            request.RequestHeader = new RequestHeader
                { TimeoutHint = TimeoutHint, ReturnDiagnostics = DiagnosticsHint, Timestamp = DateTime.UtcNow };
            return;
        }
