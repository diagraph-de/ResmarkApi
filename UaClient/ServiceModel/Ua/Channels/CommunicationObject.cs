...
/// <summary>
///     Provides a common base implementation for the basic state machine common to all communication-oriented objects in
///     the system.
/// </summary>
public abstract class CommunicationObject : ICommunicationObject
{
    ...
    /// </summary>
    /// <returns>
    ///     A value from the <see cref="T:Workstation.ServiceModel.Ua.CommunicationState" /> enumeration that indicates
    ...
    public async Task AbortAsync(CancellationToken token = default)
    {
        ...
        try
        {
            if (_aborted || State == CommunicationState.Closed) return;
        ...
        try
        {
            ...
            if (!_onClosingCalled) throw new InvalidOperationException("Channel did not call base.OnClosingAsync");

            await OnAbortAsync(token).ConfigureAwait(false);
            await OnClosedAsync(token).ConfigureAwait(false);
            if (!_onClosedCalled) throw new InvalidOperationException("Channel did not call base.OnClosedAsync");
    ...
    public async Task CloseAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            if (communicationState != CommunicationState.Closed) State = CommunicationState.Closing;
        }
        finally
        {
            _semaphore.Release();
    ...
    public async Task OpenAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            if (!_onOpeningCalled) throw new InvalidOperationException("Channel did not call base.OnOpeningAsync");

            await OnOpenAsync(token).ConfigureAwait(false);
            await OnOpenedAsync(token).ConfigureAwait(false);
            if (!_onOpenedCalled) throw new InvalidOperationException("Channel did not call base.OnOpenedAsync");

            flag2 = false;
        }
        finally
        {
            if (flag2) await FaultAsync().ConfigureAwait(false);
        }
    }
    ...
    protected async Task FaultAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            if (State == CommunicationState.Faulted) return;
    ...
    /// <summary>
    ///     Inserts processing on a communication object after it transitions to the closing state due to the invocation of the
    ///     AbortAsync operation.
    /// </summary>
    /// <param name="token">
    ///     The <see cref="T:System.Threading.CancellationToken" /> that notifies when the task should be
    ///     canceled.
    ...
    /// <summary>
    ///     Inserts processing on a communication object after it transitions to the faulted state due to the FaultAsync
    ///     operation.
    /// </summary>
    /// <param name="token">
    ///     The <see cref="T:System.Threading.CancellationToken" /> that notifies when the task should be
    ...
    protected virtual async Task OnFaulted(CancellationToken token = default)
    {
        ...
        var faulted = Faulted;
        if (faulted != null) faulted(this, EventArgs.Empty);
    }
    ...
    protected virtual async Task OnOpenedAsync(CancellationToken token = default)
    {
        ...
        try
        {
            if (_aborted || State != CommunicationState.Opening) return;
    ...
    {
        if (_exceptions.Value.TryPeek(out var ex)) return ex;
    ...
    {
        if (_exceptions.Value.TryDequeue(out var ex)) return ex;
    ...
    {
        var exception = GetPendingException();
    ...
    /// <summary>
    ///     Throws an <see cref="InvalidOperationException" /> if the communication object is not in the opening, open, or
    ///     closing state.
    /// </summary>
