...
public class ClientSessionChannel : ClientSecureChannel, ISourceBlock<PublishResponse>, IObservable<PublishResponse>
{
    ...
    ///     The user identity. Provide an <see cref="AnonymousIdentity" />,
    ///     <see cref="UserNameIdentity" />, <see cref="IssuedIdentity" /> or <see cref="X509Identity" />.
    /// </param>
    /// <param name="remoteEndpoint">
    ///     The <see cref="EndpointDescription" /> of the remote application. Obtained from a prior call to
    ///     <see
    ///         cref="DiscoveryService.GetEndpointsAsync(GetEndpointsRequest, ILoggerFactory?, UaApplicationOptions?, StackProfile?)" />
    ///     .
    /// </param>
        ...
        : base(localDescription, certificateStore, remoteEndpoint, loggerFactory, options, stackProfile)
    {
        ...
        _publishResponses = new BroadcastBlock<PublishResponse>(null,
    ...
    public ClientSessionChannel(
        ...
        ClientSessionChannelOptions? options = null,
        StackProfile? stackProfile = null)
        : base(localDescription, certificateStore,
            new EndpointDescription { EndpointUrl = endpointUrl, SecurityPolicyUri = securityPolicyUri }, loggerFactory,
            options, stackProfile)
    {
        UserIdentity = userIdentity;
        _options = options ?? new ClientSessionChannelOptions();
        _loggerFactory = loggerFactory;
    ...
    /// <param name="localDescription">The <see cref="ApplicationDescription" /> of the local application.</param>
    /// <param name="certificateStore">The local certificate store.</param>
    /// <param name="userIdentityProvider">
    ///     An asynchronous function that provides the user identity. Provide an
    ///     <see cref="AnonymousIdentity" />, <see cref="UserNameIdentity" />, <see cref="IssuedIdentity" /> or
    ///     <see cref="X509Identity" />.
    /// </param>
    /// <param name="remoteEndpoint">
    ///     The <see cref="EndpointDescription" /> of the remote application. Obtained from a prior call to
    ///     <see
    ...
    /// <summary>
    ///     Gets the asynchronous function that provides the user identity. Provide an <see cref="AnonymousIdentity" />,
    ///     <see cref="UserNameIdentity" />, <see cref="IssuedIdentity" /> or <see cref="X509Identity" />
    /// </summary>
    ...
    /// <inheritdoc />
    public PublishResponse? ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<PublishResponse> target,
        out bool messageConsumed)
    {
        return ((ISourceBlock<PublishResponse>)_publishResponses).ConsumeMessage(messageHeader, target,
            out messageConsumed);
    }
    ...
    protected override async Task OnOpeningAsync(CancellationToken token = default)
    {
        if (RemoteEndpoint.Server == null)
        {
            ...
            try
            {
                ...
                var getEndpointsRequest = new GetEndpointsRequest
                {
                    ...
                    ProfileUris = new[]
                    {
                        TransportProfileUris.UaTcpTransport
                    } // We should rethink this line, once we support transport profiles.
                };
                var getEndpointsResponse = await DiscoveryService
                    .GetEndpointsAsync(getEndpointsRequest, _loggerFactory, stackProfile: StackProfile)
        ...
        if (UserIdentityProvider != null) UserIdentity = await UserIdentityProvider(RemoteEndpoint);
    ...
    protected override async Task OnOpenAsync(CancellationToken token = default)
    {
        ...
        {
            var tokenPolicy =
                RemoteEndpoint.UserIdentityTokens?.FirstOrDefault(t => t?.TokenType == UserTokenType.IssuedToken);
            if (tokenPolicy == null) throw new ServiceResultException(StatusCodes.BadIdentityTokenRejected);

            var plainTextLength = issuedIdentity.TokenData.Length + RemoteNonce!.Length;
        ...
        {
            var tokenPolicy =
                RemoteEndpoint.UserIdentityTokens?.FirstOrDefault(t => t?.TokenType == UserTokenType.Certificate);
            if (tokenPolicy == null) throw new ServiceResultException(StatusCodes.BadIdentityTokenRejected);
        ...
        {
            var tokenPolicy =
                RemoteEndpoint.UserIdentityTokens?.FirstOrDefault(t => t?.TokenType == UserTokenType.UserName);
            if (tokenPolicy == null) throw new ServiceResultException(StatusCodes.BadIdentityTokenRejected);

            var passwordBytes = userNameIdentity.Password != null
                ? Encoding.UTF8.GetBytes(userNameIdentity.Password)
                : Array.Empty<byte>();
            var plainTextLength = passwordBytes.Length + RemoteNonce!.Length;
            IBufferedCipher encryptor;
            byte[] cipherText;
            int pos;
        ...
        {
            var tokenPolicy =
                RemoteEndpoint.UserIdentityTokens?.FirstOrDefault(t => t?.TokenType == UserTokenType.Anonymous);
            if (tokenPolicy == null) throw new ServiceResultException(StatusCodes.BadIdentityTokenRejected);
    ...
    private async Task PublishAsync(CancellationToken token = default)
    {
        var publishRequest = new PublishRequest
        {
            RequestHeader = new RequestHeader
                { TimeoutHint = _publishTimeoutHint, ReturnDiagnostics = _options.DiagnosticsHint },
    ...
    private void ThrowOnInvalidSessionServerCertificate(byte[]? sessionCertificate)
    {
        ...
        if (!string.Equals(RemoteEndpoint.SecurityPolicyUri, SecurityPolicyUris.None))
            // Verification required if the security policy for the endpoint is not None.
            compareCertificates = true;
        else if (RemoteEndpoint.UserIdentityTokens != null)
            // Check if any of the user token policies require encryption.
            foreach (var policy in RemoteEndpoint.UserIdentityTokens)
            {
                ...
                var securityPolicyUri = string.IsNullOrWhiteSpace(policy.SecurityPolicyUri)
                    ? RemoteEndpoint.SecurityPolicyUri
        ...
        {
            var isValid = RemoteEndpoint.ServerCertificate == null || sessionCertificate == null
                ? RemoteEndpoint.ServerCertificate == null &&
                  sessionCertificate == null // Valid if both certificates are null
                : RemoteEndpoint.ServerCertificate
                    .SequenceEqual(sessionCertificate); // Valid if both certificates are equal
