...
/// </summary>
/// <seealso href="https://reference.opcfoundation.org/v104/Core/docs/Part6/7.1.1/">
...
#pragma warning restore CS1998
    ...
    /// <seealso href="https://reference.opcfoundation.org/v104/Core/docs/Part6/7.1.2/">
    ///     OPC UA specification Part 6: Mappings,
    ///     7.1.2
    /// </seealso>
    public async Task<TransportConnectionOptions> OpenAsync(uint protocolVersion,
        TransportConnectionOptions localOptions, CancellationToken token)
    {
        ...
        if (count == 0) throw new ObjectDisposedException("socket");
        ...
        using (var decoder = new BinaryDecoder(new MemoryStream(receiveBuffer, 0, count, false, false)))
        {
            ...
            throw new InvalidOperationException(
                $"{nameof(UaClientConnection)}.{nameof(OpenAsync)} received unexpected message type.");
        }
    }
    ...
    public async Task<int> ReceiveAsync(byte[] buffer, int offset, int count, CancellationToken token)
    {
        ...
        if (len > maxCount) throw new ServiceResultException(StatusCodes.BadResponseTooLarge);
