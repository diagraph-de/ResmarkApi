...
/// </summary>
/// <seealso href="https://reference.opcfoundation.org/v104/Core/docs/Part6/6.7.1/">
...
public class UaSecureConversation : IConversation
{
    ...
    private readonly X509CertificateParser _certificateParser = new();
    ...
    public UaSecureConversation(ApplicationDescription localDescription, TransportConnectionOptions options,
        ICertificateStore? certificateStore, ILogger? logger)
    {
    ...
    /// <param name="logger">The logger instance.</param>
    public UaSecureConversation(uint channelId, ApplicationDescription localDescription,
        TransportConnectionOptions options, ICertificateStore? certificateStore, ILogger? logger)
    ...
    /// <inheritdoc />
    public Task EncryptMessageAsync(Stream bodyStream, uint messageType, uint requestHandle,
        Func<byte[], int, int, CancellationToken, Task> consume, CancellationToken token)
    {
    ...
    /// <inheritdoc />
    public async Task<(uint messageType, uint requestHandle)> DecryptMessageAsync(Stream bodyStream,
        Func<byte[], int, int, CancellationToken, Task<int>> receive, CancellationToken token)
    {
        uint requestId;
        ...
        do
        {
            ...
            using (var decoder = new BinaryDecoder(stream))
            {
                ...
                var messageLength = (int)decoder.ReadUInt32(null);
                ...
                switch (messageType)
                {
                    ...
                    case MessageTypes.CLOF:
                        ...
                        if (tokenId != _currentRemoteTokenId)
                        {
                            ...
                            if (_symIsSigned)
                            {
                                ...
                                if (_symIsEncrypted)
                                    _symDecryptor!.Init(
                                        false,
                                        new ParametersWithIV(new KeyParameter(_remoteEncryptingKey),
                                            _remoteInitializationVector));
                            }
                        ...
                        if (_symIsEncrypted)
                        {
                            var inputCount = messageLength - plainHeaderSize;
                            Debug.Assert(inputCount % _symDecryptor!.GetBlockSize() == 0,
                                "Input data is not an even number of encryption blocks.");
                            _symDecryptor.DoFinal(_receiveBuffer, plainHeaderSize, inputCount, _receiveBuffer,
                                plainHeaderSize);
                        }
                        ...
                        if (_symIsSigned)
                        {
                            ...
                            var signature = new byte[_symVerifier.GetMacSize()];
                        ...
                        if (_symIsEncrypted)
                        {
                            ...
                            {
                                paddingHeaderSize = 2;
                                paddingSize = BitConverter.ToInt16(_receiveBuffer!,
                                    messageLength - _symSignatureSize - paddingHeaderSize);
                            }
                            else
                            {
                                paddingHeaderSize = 1;
                            ...
                            bodySize = messageLength - plainHeaderSize - _sequenceHeaderSize - paddingSize -
                                       paddingHeaderSize - _symSignatureSize;
                        }
                        else
                        {
                    ...
                    case MessageTypes.OPNF:
                        // header
                        channelId = decoder.ReadUInt32(null);
                        if (!IsServer) ChannelId = channelId;
                        ...
                        if (IsServer)
                            await SetRemoteCertificateAsync(securityPolicyUri, remoteCertificate, token)
                        ...
                        if (_asymIsEncrypted)
                        {
                            var cipherTextBlock = new byte[_asymLocalCipherTextBlockSize];
                            var jj = plainHeaderSize;
                            for (var ii = plainHeaderSize; ii < messageLength; ii += _asymLocalCipherTextBlockSize)
                            {
                                Buffer.BlockCopy(_receiveBuffer!, ii, cipherTextBlock, 0,
                                    _asymLocalCipherTextBlockSize);

                                // decrypt with local private key.
                                var plainTextBlock = _asymDecryptor!.DoFinal(cipherTextBlock);
                                Debug.Assert(plainTextBlock.Length == _asymLocalPlainTextBlockSize,
                                    "Decrypted block length was not as expected.");
                        ...
                        if (_asymIsSigned)
                        {
                            ...
                            if (!_asymVerifier.VerifySignature(_receiveBuffer!
                                    .AsArraySegment(datalen, _asymRemoteSignatureSize).ToArray()))
                                throw new ServiceResultException(StatusCodes.BadSecurityChecksFailed);
                        }
                        ...
                        if (_asymIsEncrypted)
                        {
                            if (_asymLocalCipherTextBlockSize > 256)
                            {
                                paddingHeaderSize = 2;
                                paddingSize = BitConverter.ToInt16(_receiveBuffer!,
                                    messageLength - _asymRemoteSignatureSize - paddingHeaderSize);
                            }
                            else
                            {
                                paddingHeaderSize = 1;
                                paddingSize =
                                    _receiveBuffer![messageLength - _asymRemoteSignatureSize - paddingHeaderSize];
                            }

                            bodySize = messageLength - plainHeaderSize - _sequenceHeaderSize - paddingSize -
                                       paddingHeaderSize - _asymRemoteSignatureSize;
                        }
                        else
                        {
                    ...
                    case MessageTypes.MSGA:
                        ...
                        if (message != null) throw new ServiceResultException(statusCode, message);
        ...
        if (remoteCertificate != null)
        {
            ...
            if (cert != null)
            {
                if (_certificateStore != null)
                {
                    ...
                    if (!result)
                        throw new ServiceResultException(StatusCodes.BadSecurityChecksFailed,
                            "Remote certificate is untrusted.");
                }
    ...
    private async Task EncryptRequestMessage(Stream bodyStream, uint messageType, uint requestHandle,
        Func<byte[], int, int, CancellationToken, Task> consume, CancellationToken token)
    {
        var chunkCount = 0;
        var bodyCount = (int)(bodyStream.Length - bodyStream.Position);
        while (bodyCount > 0)
        {
            chunkCount++;
            if (_options.MaxChunkCount > 0 && chunkCount > _options.MaxChunkCount)
            ...
            using (var encoder = new BinaryEncoder(stream))
            {
                ...
                if (TokenId != _currentLocalTokenId)
                {
                    ...
                    if (_symIsSigned)
                    {
                        ...
                        if (_symIsEncrypted)
                            _symEncryptor!.Init(
                                true,
                                new ParametersWithIV(new KeyParameter(_localEncryptingKey),
                                    _localInitializationVector));
                    }
                }

                var plainHeaderSize = encoder.Position;
                ...
                if (_symIsEncrypted)
                {
                    ...
                    maxBodySize =
                        ((int)_options.ReceiveBufferSize - plainHeaderSize) / _symEncryptionBlockSize *
                        _symEncryptionBlockSize - _sequenceHeaderSize - paddingHeaderSize - _symSignatureSize;
                    if (bodyCount < maxBodySize)
                    {
                        bodySize = bodyCount;
                        paddingSize =
                            (_symEncryptionBlockSize -
                             (_sequenceHeaderSize + bodySize + paddingHeaderSize + _symSignatureSize) %
                             _symEncryptionBlockSize) % _symEncryptionBlockSize;
                    }
                    else
                    {
                    ...
                    chunkSize = plainHeaderSize + _sequenceHeaderSize + bodySize + paddingSize + paddingHeaderSize +
                                _symSignatureSize;
                }
                else
                {
                    paddingHeaderSize = 0;
                    paddingSize = 0;
                    maxBodySize = (int)_options.ReceiveBufferSize - plainHeaderSize - _sequenceHeaderSize -
                                  _symSignatureSize;
                    if (bodyCount < maxBodySize)
                        bodySize = bodyCount;
                    else
                ...
                if (_symIsEncrypted)
                {
                    ...
                    for (var i = 0; i < paddingSize; i++) encoder.WriteByte(null, paddingByte);
                ...
                if (_symIsSigned)
                {
                    ...
                    var signature = new byte[_symSigner.GetMacSize()];
                ...
                if (_symIsEncrypted)
                {
                    var inputCount = position - plainHeaderSize;
                    Debug.Assert(inputCount % _symEncryptor!.GetBlockSize() == 0,
                        "Input data is not an even number of encryption blocks.");
    ...
    private async Task EncryptOpenMessage(Stream bodyStream, uint messageType, uint requestHandle,
        Func<byte[], int, int, CancellationToken, Task> consume, CancellationToken token)
    {
        var chunkCount = 0;
        var bodyCount = (int)(bodyStream.Length - bodyStream.Position);
        while (bodyCount > 0)
        {
            ...
            using (var encoder = new BinaryEncoder(stream))
            {
                ...
                if (SecurityMode != MessageSecurityMode.None)
                {
                    ...
                    var thumbprint = new byte[_thumbprintDigest!.GetDigestSize()];
                ...
                if (_asymIsEncrypted)
                {
                    ...
                    maxBodySize =
                        ((int)_options.ReceiveBufferSize - plainHeaderSize) / _asymRemoteCipherTextBlockSize *
                        _asymRemotePlainTextBlockSize - _sequenceHeaderSize - paddingHeaderSize -
                        _asymLocalSignatureSize;
                    if (bodyCount < maxBodySize)
                    {
                        bodySize = bodyCount;
                        paddingSize =
                            (_asymRemotePlainTextBlockSize -
                             (_sequenceHeaderSize + bodySize + paddingHeaderSize + _asymLocalSignatureSize) %
                             _asymRemotePlainTextBlockSize) % _asymRemotePlainTextBlockSize;
                    }
                    else
                    {
                        bodySize = maxBodySize;
                    ...
                    chunkSize = plainHeaderSize +
                                (_sequenceHeaderSize + bodySize + paddingSize + paddingHeaderSize +
                                 _asymLocalSignatureSize) / _asymRemotePlainTextBlockSize *
                                _asymRemoteCipherTextBlockSize;
                }
                else
                {
                    paddingHeaderSize = 0;
                ...
                if (_asymIsSigned)
                {
                    ...
                    var signature = _asymSigner.GenerateSignature();
                ...
                if (_asymIsEncrypted)
                {
                    ...
                    var plainText = new byte[_asymRemotePlainTextBlockSize];
                    var jj = plainHeaderSize;
                    for (var ii = plainHeaderSize; ii < position; ii += _asymRemotePlainTextBlockSize)
                    {
                        Buffer.BlockCopy(_sendBuffer, ii, plainText, 0, _asymRemotePlainTextBlockSize);

                        // encrypt with remote public key.
                        var cipherText = _asymEncryptor!.DoFinal(plainText);
                        Debug.Assert(cipherText.Length == _asymRemoteCipherTextBlockSize,
                            nameof(_asymRemoteCipherTextBlockSize));
    ...
    private void UpdateKeys()
    {
        if (_symIsSigned)
        {
            ...
            var localSecurityKey = CalculatePSHA(remoteNonce, localNonce,
                _symSignatureKeySize + _symEncryptionKeySize + _symEncryptionBlockSize, _securityPolicyUri!);
            ...
            Buffer.BlockCopy(localSecurityKey, _symSignatureKeySize + _symEncryptionKeySize,
                _localInitializationVector!, 0, _symEncryptionBlockSize);

            // (re)create remote security keys for decrypting the next message received that has a new TokenId
            var remoteSecurityKey = CalculatePSHA(localNonce, remoteNonce,
                _symSignatureKeySize + _symEncryptionKeySize + _symEncryptionBlockSize, _securityPolicyUri!);
            ...
            Buffer.BlockCopy(remoteSecurityKey, _symSignatureKeySize + _symEncryptionKeySize,
                _remoteInitializationVector!, 0, _symEncryptionBlockSize);
        }
    }
    ...
    private static byte[] CalculatePSHA(byte[] secret, byte[] seed, int sizeBytes, string securityPolicyUri)
    {
        ...
        var mac = new HMac(digest);
        var output = new byte[sizeBytes];
        mac.Init(new KeyParameter(secret));
        var a = seed;
        var size = digest.GetDigestSize();
        var iterations = (output.Length + size - 1) / size;
        var buf = new byte[mac.GetMacSize()];
        var buf2 = new byte[mac.GetMacSize()];
