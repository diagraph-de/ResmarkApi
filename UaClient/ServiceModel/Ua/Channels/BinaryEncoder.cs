...
public sealed class BinaryEncoder : IEncoder, IDisposable
{
    ...
    public void WriteExpandedNodeId(string? fieldName, ExpandedNodeId? value)
    {
        ...
        var ns = value.NodeId.NamespaceIndex;
        var svr = value.ServerIndex;
        byte b = 0x00;
        ...
        if ((b & 0x80) != 0) WriteString(null, value.NamespaceUri);

        if ((b & 0x40) != 0) WriteUInt32(null, svr);
    }
    ...
    public void WriteDiagnosticInfo(string? fieldName, DiagnosticInfo? value)
    {
        ...
        byte b = 0;
        if (value.SymbolicId >= 0) b |= 1;

        if (value.NamespaceUri >= 0) b |= 2;

        if (value.Locale >= 0) b |= 8;

        if (value.LocalizedText >= 0) b |= 4;

        if (value.AdditionalInfo != null) b |= 16;

        if (value.InnerStatusCode != 0u) b |= 32;

        if (value.InnerDiagnosticInfo != null) b |= 64;

        WriteByte(null, b);
        if ((b & 1) != 0) WriteInt32(null, value.SymbolicId);

        if ((b & 2) != 0) WriteInt32(null, value.NamespaceUri);

        if ((b & 8) != 0) WriteInt32(null, value.Locale);

        if ((b & 4) != 0) WriteInt32(null, value.LocalizedText);

        if ((b & 16) != 0) WriteString(null, value.AdditionalInfo);

        if ((b & 32) != 0) WriteStatusCode(null, value.InnerStatusCode);

        if ((b & 64) != 0) WriteDiagnosticInfo(null, value.InnerDiagnosticInfo);
    }
    ...
    public void WriteLocalizedText(string? fieldName, LocalizedText? value)
    {
        ...
        byte b = 0;
        if (value.Locale != null) b |= 1;

        if (value.Text != null) b |= 2;

        WriteByte(null, b);
        if ((b & 1) != 0) WriteString(null, value.Locale);

        if ((b & 2) != 0) WriteString(null, value.Text);
    }
    ...
    public void WriteVariant(string? fieldName, Variant value)
    {
        ...
        var b = (byte)value.Type;
        var dims = value.ArrayDimensions;
        if (dims == null)
        {
            WriteByte(null, b);
            switch (value.Type)
    ...
    public void WriteDataValue(string? fieldName, DataValue? value)
    {
        ...
        byte b = 0;
        if (!Variant.IsNull(value.Variant)) b |= 1;

        if (value.StatusCode != 0u) b |= 2;

        if (value.SourceTimestamp != DateTime.MinValue) b |= 4;

        if (value.SourcePicoseconds != 0) b |= 16;

        if (value.ServerTimestamp != DateTime.MinValue) b |= 8;

        if (value.ServerPicoseconds != 0) b |= 32;

        WriteByte(null, b);
        if ((b & 1) != 0) WriteVariant(null, value.Variant);

        if ((b & 2) != 0) WriteStatusCode(null, value.StatusCode);

        if ((b & 4) != 0) WriteDateTime(null, value.SourceTimestamp);

        if ((b & 16) != 0) WriteUInt16(null, value.SourcePicoseconds);

        if ((b & 8) != 0) WriteDateTime(null, value.ServerTimestamp);

        if ((b & 32) != 0) WriteUInt16(null, value.ServerPicoseconds);
    }
    ...
    private bool TryWriteArrayLength<T>([NotNullWhen(true)] T[]? values)
    {
        if (values == null)
        {
            WriteInt32(null, -1);
