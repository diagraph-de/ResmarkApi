...
{
    private readonly X509CertificateParser _certParser = new();
    ...
    /// <param name="createLocalCertificateIfNotExist">
    ///     Set true to create a local certificate and private key, if the files do
    ///     not exist.
    /// </param>
    public DirectoryStore(string path, bool acceptAllRemoteCertificates = true,
        bool createLocalCertificateIfNotExist = true)
    {
    ...
    /// <inheritdoc />
    public async Task<(X509Certificate? Certificate, RsaKeyParameters? Key)> GetLocalCertificateAsync(
        ApplicationDescription applicationDescription, ILogger? logger = null, CancellationToken token = default)
    {
        if (applicationDescription == null) throw new ArgumentNullException(nameof(applicationDescription));

        var applicationUri = applicationDescription.ApplicationUri;
        if (string.IsNullOrEmpty(applicationUri))
            throw new ArgumentOutOfRangeException(nameof(applicationDescription),
                "Expecting ApplicationUri in the form of 'http://{hostname}/{appname}' -or- 'urn:{hostname}:{appname}'.");

        string? subjectName = null;
        string? hostName = null;
        string? appName = null;

        var appUri = new UriBuilder(applicationUri);
        ...
        if (ownCertsInfo.Exists)
            foreach (var info in ownCertsInfo.EnumerateFiles())
                using (var crtStream = info.OpenRead())
                {
                    ...
                    if (c != null) ownCerts.Add(c);
                }

        var ownCertStore =
            X509StoreFactory.Create("Certificate/Collection", new X509CollectionStoreParameters(ownCerts));
        ...
        if (crt != null)
        {
            ...
            if (asn1OctetString != null)
            {
                ...
                if (gns.GetNames().Any(n =>
                        n.TagNo == GeneralName.UniformResourceIdentifier && n.Name.ToString() == applicationUri))
                {
                    ...
                    if (ki.Exists)
                        using (var keyStream = new StreamReader(ki.OpenRead()))
                        {
                            ...
                            if (keyPair != null) key = keyPair.Private as RsaKeyParameters;
                        }
                }
            }
        }
        ...
        if (!CreateLocalCertificateIfNotExist) return (null, null);

        // Create new certificate
        var subjectDN = new X509Name(subjectName);

        // Create a keypair.
        var kp = await Task.Run(() =>
        {
        ...
        cg.AddExtension(
            ...
            false,
            new AuthorityKeyIdentifier(SubjectPublicKeyInfoFactory.CreateSubjectPublicKeyInfo(kp.Public),
                new GeneralNames(new GeneralName(subjectDN)), subjectSN));
        ...
        cg.AddExtension(
            ...
            true,
            new KeyUsage(KeyUsage.DataEncipherment | KeyUsage.DigitalSignature | KeyUsage.NonRepudiation |
                         KeyUsage.KeyCertSign | KeyUsage.KeyEncipherment));
        ...
        if (crtInfo.Exists) crtInfo.Delete();
    ...
    {
        if (AcceptAllRemoteCertificates) return Task.FromResult(true);
        ...
        if (IsSelfSigned(target))
        {
            ...
            var trustedCertStore = X509StoreFactory.Create("Certificate/Collection",
                new X509CollectionStoreParameters(trustedCerts));
            if (trustedCertStore.GetMatches(selector).Count > 0) return Task.FromResult(true);
    ...
    private static PkixCertPathBuilderResult VerifyCertificate(X509Certificate target, HashSet trustedRootCerts,
        HashSet intermediateCerts)
    {
        ...
        var intermediateCertStore = X509StoreFactory.Create("Certificate/Collection",
            new X509CollectionStoreParameters(intermediateCerts));
        pkixParams.AddStore(intermediateCertStore);

        // Build and verify the certification chain
        var builder = new PkixCertPathBuilder();
        var result = builder.Build(pkixParams);
        return result;
    }
