...
{
    private static readonly Dictionary<Type, SimpleAttributeOperand[]> _selectClauseCache = new();
    private static readonly Dictionary<Type, PropertyInfo[]> _deserializerCache = new();
    ...
    {
        var type = typeof(T);
        if (_selectClauseCache.TryGetValue(type, out var clauses)) return clauses;
    ...
    private static void RegisterSelectClauseAndDeserializer(Type type)
    {
        ...
        foreach (var info in type.GetRuntimeProperties())
        {
            ...
            if (efa == null || string.IsNullOrEmpty(efa.TypeDefinitionId)) continue;

            var clause = new SimpleAttributeOperand
            {
                TypeDefinitionId = NodeId.Parse(efa.TypeDefinitionId!),
                BrowsePath = !string.IsNullOrWhiteSpace(efa.BrowsePath)
                    ? efa.BrowsePath!.Split('/').Select(s => QualifiedName.Parse(s)).ToArray()
                    : Array.Empty<QualifiedName>(),
